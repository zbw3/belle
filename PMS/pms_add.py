...
def pms_list():
    ...
    response = requests.post(
        'https://retail-test.belle.cn/pms/bill_replenish_order/list2.json',params=params,cookies=cookies,headers=headers,data=data,)
...
def pms_confirm():
    ...
    response = requests.post(
        'https://retail-test.belle.cn/pms/bill_replenish_order_dtl_confirm/verification',cookies=cookies,headers=headers,data=data,)
    print(response.text)
...
def update_status():
    ...
    response = requests.post(
        'https://retail-test.belle.cn/pms/bill_replenish_order/updateStatusById',cookies=cookies,headers=headers,data=data,)
    print(response.text)
...
def pms_purchase_order_generation(bill_id):
    ...
    response_1 = requests.post(
        'https://retail-test.belle.cn/pms/purchase_order_generation/getOrderOrReplenishItem.json',cookies=cookies,headers=headers,data=data,)
    res_json_1 = json.loads(response_1.text)
    item = res_json_1['rows'][0]
    # 查询store
    response_2 = requests.post(
    'https://retail-test.belle.cn/pms/purchase_order_generation/getOrderOrReplenishStore.json',cookies=cookies,headers=headers,data=data,)
...
def purchase_order_generation():
    ...
    response = requests.post(
        'https://retail-test.belle.cn/pms/purchase_order_generation/batchGeneration',cookies=cookies,headers=headers,data=data,)
    # print(response.text)
...
def search_bill_no():
    ...
    # 查询bill_no的id
    response_1 = requests.post(
        'https://retail-test.belle.cn/pms/bill_purchase_order/selectPurchaseOrder',params=params,cookies=cookies,headers=headers,data=data,)
    ...
    response_2 = requests.post(
        'https://retail-test.belle.cn/pms/bf_purchase_setting/getInfoByBillNos',cookies=cookies,headers=headers,data=data,)
...
def batch_audit():
    ...
    response = requests.post(
        'https://retail-test.belle.cn/pms/bill_purchase_order/batchAudit',cookies=cookies,headers=headers,data=data,)
    print(f"采购订单" + response.json()['msg'])




# 使用方法，把批量导入生成的订补单号复制到raw_data里面，第一个三双引号右边要有值。还要复制一下cookie过来到config文件里。
"""批量导入时获得的单据编码换到下面的值里面"""
raw_data = """D029BH260021
D100BH260008
"""
...
for bill_id in bill_ids:
    # pms_purchase_order_generation() # 批量生成的查询接口，不用打开
    # purchase_order_generation() # 批量生成接口，不用打开
    batch_audit() # 提交批量生成和审核接口
...
"""
D085-天津BL--海外业务部-华北
D043-内蒙BL-品牌供应链-华北呼市鞋仓-华北
E259-合肥BL-海外业务部-鲁豫合肥一仓-鲁豫皖
D100-山西BL-海外业务部-华北太原鞋仓-华北
C054-吉林BL--品牌供应链-东北
I271-上海多品--无标签-东部
D029-北京BL-市场营销部-华北北京鞋仓-华北
D057-唐秦BL-意礴、品牌供应链-华北唐山鞋仓-华北
"""